...
/* Build a derived datatype for distributing the input data */
 void Build_mpi_type(long double* a_p, long double* b_p, int* n_p,
      MPI_Datatype* input_mpi_t_p);

/* Calculate local integral  */
 long double Trap(long double left_endpt, long double right_endpt, int trap_count, 
   long double base_len);    

/* Function we're integrating */
 long double f(long double x); 

int Runtrap(long double a, long double b, int n);
